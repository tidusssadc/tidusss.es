...
test('Jinx: ya tiene build de referencia real (parche 26.17) — responde con ella, nunca con contenido de Lucian mezclado', async () => {
  const payload = await asJson(await onRequest({ request: post({ question: '¿Cuál es la build de Jinx?' }) }));
  assert.equal(payload.ok, true);
  if (payload.ok) {
    assert.equal(payload.data.status, 'sufficient');
    assert.ok(payload.data.sources.length > 0);
    assert.ok(payload.data.sources.some((source) => source.type === 'Build'));
    assert.ok(!payload.data.sources.some((source) => source.url.includes('lucian')));
  }
});
