...
> **Estado:** v1.1 · Auditoría completa del código en la rama `main`, commit `4bcbee7` (24 jul. 2026). Actualizado el 24 jul. 2026 para incorporar el diseño de dominio de **The League Laboratory** (§5.13, §6 Capítulo III, ADR-004) — ver [`docs/league-laboratory.md`](league-laboratory.md) para el detalle completo.
...
**Estado real:** solo dominio. Sin rutas públicas, sin componentes visuales, sin datos reales, sin endpoints, sin ningún cambio en Home, Live, Match History o APIs existentes. Verificado con `astro check`/`eslint`/`build` tras la creación (§ADR-004). La primera herramienta a construir sobre esta arquitectura será la Tier List (roadmap interno en `docs/league-laboratory.md` §8).
...
### Chapter III — League Laboratory (dominio diseñado, implementación pendiente)
...
- **Arquitectura:** **completada la capa de dominio** — `src/domain/league-laboratory/` (10 entidades, value objects `EditorialTake`/`KnowledgeScope`, registro con funciones de consulta, puente al Content Graph). Detalle completo en [`docs/league-laboratory.md`](league-laboratory.md). El ambiente `tier-list` ya definido en el Environment Engine sigue disponible; se recomienda (no implementado) unificarlo en un único ambiente `'laboratory'` compartido por todas las herramientas del capítulo.
- **Dependencias:** Content Graph (satisfecha — 6 de 10 entidades bridgean con `ContentEntityKind` ya reservados). Pendiente: decidir si `rune-page`, `synergy`, `concept` y `meta-state` necesitan su propio `ContentEntityKind` (requeriría una ADR que sí toque `content-graph/types.ts`) **[Decisión pendiente, ver docs/league-laboratory.md §6]**. Definir si el contenido es 100% editorial manual o se apoya en datos de parche de Data Dragon/Riot **[Decisión de producto pendiente]**.
- **Impacto:** todavía ninguno visible — no hay rutas ni UI. Cuando se implemente la primera herramienta (Tier List, siguiente paso natural según el roadmap interno del dominio), será el primer capítulo que añade rutas nuevas de contenido indexable — impacto directo en SEO (§9) y en el mantenimiento manual de `sitemap.xml`.
