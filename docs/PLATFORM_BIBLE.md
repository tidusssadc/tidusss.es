...
### ADR-012 — 2026-07-26 — Guía editorial completa de campeón: Lucian como primera aplicación real

**Contexto:** se encargó convertir `/campeones/lucian` en "el mejor recurso en español para aprender el campeón", con una filosofía explícita ("no te digo qué construir, te explico por qué") y 17 secciones (build, builds alternativas, orden de habilidades, runas, objetos situacionales, power spikes, matchups fáciles/difíciles, sinergias, errores comunes, consejos rápidos, conceptos, vídeos, partidas, cambios recientes, historial editorial), con la instrucción explícita de no inventar contenido y de diseñar todo pensando en reutilizarlo para Kai'Sa, Ashe, Jhin, Varus, etc.
**El problema real, descubierto en la auditoría previa:** `Build`, `RunePage`, `Matchup`, `Synergy` y `Concept` existían como tipos desde ADR-004 (Fase 0) pero **nunca se habían poblado con datos reales** — cero archivos en `src/data/league-laboratory/` para ninguno de los cinco, y `[slug].astro` ni siquiera sembraba esas colecciones en `buildLabRegistry()`. La página anterior no tenía nada que mostrar en esas secciones más allá de listas `<li>` vacías.
**Decisión — modelo de dominio:** se rediseñaron `Build` y `RunePage` (sin necesidad de migración: cero datos reales existían) para que cada objeto/runa sea un `BuildItemChoice`/`RuneChoice` con `reasoning` obligatorio (opcional solo en runas menores) en vez de un id suelto — "un objeto sin razonamiento no pertenece a una build editorial". `Build` gana `variant: 'primary' | 'situational'` y `situationalContext`, resolviendo a la vez "build principal", "builds alternativas" y "objetos situacionales — cuándo comprar cada uno" con una sola estructura. `Matchup` gana `tips`, `videoUrl`, `matchHref` opcionales. Se añadió `LabChampion.editorialHistory` (real, con fecha ISO) y `LabChampion.coreConceptIds` — esta última es la vía por la que un campeón se enlaza directamente con los conceptos del Laboratorio que su propio `profile` ya evidencia, distinta y fusionada con la vía ya existente (`getRelatedConceptsFor`, derivada de matchups/guías/sinergias) para que "Conceptos importantes" tenga una sola fuente de verdad combinada.
**Decisión — sin contenido inventado:** `builds.ts`, `rune-pages.ts`, `matchups.ts`, `synergies.ts` se crearon **vacíos a propósito** (con la forma de una entrada real documentada en comentario, para cuando exista). No hay ninguna build, página de runas, matchup o sinergia real de Lucian todavía — nadie tiene ese conocimiento salvo Tidusss, y rellenarlo con una suposición razonable habría sido exactamente lo que el encargo prohibía. `concepts.ts` sí se pobló con las 6 definiciones reales y objetivas que pedía el encargo (Spacing, Power Spike, Snowball, Trading, Tempo, Wave Management) — vocabulario ya establecido de League of Legends, no opinión editorial inventada — y Lucian se enlaza directamente a 4 de ellas (`coreConceptIds`) porque su `profile` ya existente las evidencia con claridad (fortalezas/debilidades/power spikes/errores comunes ya publicados en Fase 2); no se enlazan Tempo ni Wave Management porque el perfil actual no dice nada específico sobre ellos. El historial editorial de Lucian usa solo fechas y hechos reales ya documentados en este mismo log de ADRs (24 y 26 jul. 2026).
**Decisión — "Partidas donde aparece":** a diferencia del resto de la página (contenido editorial estático), esta sección sí tiene una fuente de datos real y viva: `RecentMatch.championName` ya existe en el pipeline Riot Match-V5 (`/api/riot/overview`, usado hoy por Live). Se construyó `ChampionMatchHistory.astro`, genérico por `championName`, que hace fetch en cliente y filtra `recent.matches`/`today.matches` — cero cambios de backend. De paso, esto da un uso real a `RiotSkeleton.astro`/`RiotUnavailable.astro`/`MatchRow.astro`, marcados como código huérfano en ADR-002: los dos primeros se reutilizan tal cual; el tercero no se pudo importar directamente (es un componente Astro, solo renderizable en build/servidor, y estos datos solo existen en tiempo de petición) pero su forma de marcado (clases `.riot-match*`, ya con CSS real) se replicó en un `<template>` clonado por script, el mismo patrón que ya usa Live para `MatchCard.astro`. **Vídeos relacionados** no tuvo el mismo tratamiento: se auditó a fondo (`YouTubeVideo`, `matchVideoLinks`) y no existe ningún campo ni dataset que asocie un vídeo a un campeón — mantiene el estado honesto "Pendiente" ya existente desde Fase 2, sin inventar una relación.
**Diseño:** la rejilla densa de 2 columnas (`champion-knowledge-grid`) se sustituyó por `champion-guide-sections`, una sola columna con `gap: 4rem` — "mucho aire", lectura de guía editorial, no panel de estadísticas. Los 9 componentes nuevos (`BuildBlock`, `BuildItemCard`, `RuneBlock`, `MatchupCard`, `SynergyCard`, `ConceptCard`, `EditorialHistoryTimeline`, `EditorialTakeBlock`, `MatchupDifficultyBadge`, `ChampionMatchHistory`) no reciben nunca el nombre "Lucian" en su código — todos toman el campeón como prop, verificado manualmente sirviendo la misma página para Kai'Sa (borrador) y un campeón sin curar (Aatrox), donde cada sección degrada a su estado vacío correctamente.
**SEO:** título («Lucian ADC: Guía, Build y Runas») y meta-descripción reescritos para cubrir de forma natural "Lucian ADC", "Guía Lucian", "Build Lucian", "Lucian Master" y "Lucian EUW", solo con hechos ya reales (Tidusss, Master ADC EUW). Se añadió `Article` en JSON-LD, **solo cuando `hasProfile` es cierto** (no tiene sentido marcar como artículo una ficha que dice "Pendiente" en casi todo), con `datePublished`/`dateModified` tomados del propio `editorialHistory` real — nunca inventados.
**Consecuencia:** la arquitectura completa de una guía de campeón de nivel "editorial premium" queda construida y probada contra datos reales de un solo campeón (Lucian), lista para recibir contenido genuino (build, runas, matchups, sinergias) de Tidusss sin ningún cambio de código, y reutilizable sin modificación para cualquier otro campeón curado. El contenido de fondo (qué comprar, qué runas, qué matchups son difíciles) sigue pendiente — es conocimiento que solo Tidusss tiene y que esta fase no podía ni debía inventar.
**Documentación relacionada:** [`docs/league-laboratory.md`](league-laboratory.md) §16.

### ADR-013 — 2026-07-26 — Contenido real de Tidusss para Lucian, parche 26.14: build, runas y sinergias

**Contexto:** con la arquitectura de la guía de Lucian ya construida (ADR-012) pero sin ningún dato real, se encargó completarla con el criterio genuino de Tidusss para el parche 26.14, aportado íntegramente dentro del propio encargo — sin consultar fuentes externas para decidir build, runas, matchups, sinergias o power spikes, y sin inventar nada más allá de lo dado.
**Hallazgo que obligó a un cambio mínimo de tipos:** el modelo de `BuildItemChoice`/`RuneChoice` (ADR-012) exigía un `itemId`/`runeId` numérico obligatorio — pensado para resolver iconos de Data Dragon. El contenido real aportado da nombres ("Segador de Esencia", "Ataque Intensificado"), nunca ids numéricos, y consultarlos habría exigido una fuente externa prohibida en este encargo. Se añadió `name: string` (obligatorio, el dato real) a ambos tipos y se convirtió `itemId`/`runeId` en opcional — un cambio mínimo, no una redefinición: la forma de "razonamiento por elección" no cambia, solo qué campo es la fuente de verdad cuando no hay id. Por el mismo motivo, `RunePage.primaryTreeId`/`secondaryTreeId` pasaron a opcionales — no se conoce (ni se ha buscado) el árbol secundario de Lucian, y el tipo no puede exigir un dato que el encargo explícitamente prohíbe rellenar con una suposición. Se añadió también `Build.boots?: BuildItemChoice[]`, un slot propio para botas: no encajaban ni en "objetos iniciales" ni en "objetos situacionales" (la elección de botas es independiente de qué segundo objeto se compre) y el encargo pedía presentarlas sin declarar ninguna superior a la otra.
**Decisión — dos builds, no una con una nota al margen:** se representan "ruta más sólida" y "ruta personal de Tidusss" como dos `Build` completos (`variant: 'primary'` / `'situational'`), reutilizando sin cambios el mecanismo ya diseñado en ADR-012. El riesgo de la ruta personal (retrasar Filo Infinito si el tercer objeto exige penetración) se documenta en dos sitios reforzándose mutuamente: como `cons` del objeto Navori y como el `editorialTake` completo de esa build — el bloque ya construido en ADR-012 con mayor peso visual (borde dorado), sin necesidad de ningún componente nuevo. La tensión "recomendación general" vs. "preferencia personal" se hace visualmente evidente con tres señales no dependientes del color: la etiqueta del badge ("Build principal"/"Build alternativa"), un texto explícito bajo el título (`situationalContext`, ahora un _callout_ con borde e icono, no solo cursiva) y el propio `editorialTake`.
**Decisión — solo se completa lo confirmado:** `RunePage` de Lucian tiene únicamente la runa principal (Ataque Intensificado); `secondaryRunes`/`statShards` quedan como arrays vacíos, mostrados en la interfaz como "Pendiente de análisis" explícito — nunca omitidos en silencio ni completados con una elección genérica. Las 6 sinergias confirmadas (Milio, Nami, Yuumi, Braum, Nautilus, Pyke) se modelan sin ninguna clasificación numérica ni tier, con una única `editorialTake` compartida y deliberadamente breve ("recomendada por Tidusss; el análisis detallado todavía está pendiente") — exactamente lo que el encargo autorizaba, ni más ni menos.
