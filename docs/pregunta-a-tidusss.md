...
> **Estado:** propuesta v0.1 original — histórico, conservado tal cual para trazabilidad de las decisiones de diseño.
> **Actualización (2026-08-03):** la V1 pública descrita como objetivo en este documento ya está construida y publicada. Ruta real: `/pregunta`. Endpoint real: `POST /api/pregunta` (`functions/api/pregunta.ts`). Motor: el determinista (`domain/knowledge-answering`), **sin LLM**, tal y como exige este documento en su §11 y §13. Contrato público real: `domain/knowledge-answering/public.ts` (`PublicAnswer`), no el ejemplo ilustrativo de §5 — nunca copiado literalmente, construido reutilizando los tipos existentes (`EditorialConfidence`, `RetrievalCoverage`). Integrada en Home y en `/campeones/lucian`, y en la navegación principal como "Pregunta" (no "IA"/"Chat"/"Asistente"). Ver ADR-018 en [`PLATFORM_BIBLE.md`](PLATFORM_BIBLE.md) y la actualización de estado en [`knowledge-answering.md`](knowledge-answering.md) §11.
...
> **Nota de estado (2026-08-03):** las 5 capas de arriba ya tienen una primera versión real e implementada — `src/domain/knowledge-generation/` y [`docs/knowledge-generation.md`](knowledge-generation.md). La capa 1 (puerta de alcance) la resuelve hoy la Recuperación léxica, no un embedding — Claude, cuando está configurado, nunca llega a invocarse para una pregunta que la Recuperación ya clasificó como fuera de alcance o sin cobertura. Las capas 2-5 se implementan literalmente como se describen aquí: prompt de sistema con contexto cerrado, salida forzada por tool-use, validación de citas en servidor (más comprobaciones adicionales: sin URLs inventadas, sin filtración de instrucciones, sin inflación de confianza detectada en el propio texto), y una garantía estructural extra que este diseño no había anticipado: `confidence`/`coverage`/`patchId`/`editorialDate` ni siquiera existen como campos que el modelo pueda devolver — no hace falta "acotarlos", es imposible que aparezcan. Sigue sin existir el modelo de embeddings de la puerta de alcance (§3.2.1), la caché de respuestas, y por supuesto Claude sigue sin activarse en producción — todo esto queda documentado como pendiente en `knowledge-generation.md` §11.
